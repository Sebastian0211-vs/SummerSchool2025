...
def circle_intersections_expanded(A1, B1, C1, A2, B2, C2):
    """
    ...
    # Check if circles are identical (all coefficients are zero)
    if abs(a) < 1e-10 and abs(b) < 1e-10 and abs(c) < 1e-10:
        return []  # Circles are identical - infinite intersections
    
    # Check if lines are parallel (a = b = 0 but c != 0)
    if abs(a) < 1e-10 and abs(b) < 1e-10:
        return []  # No intersections - circles don't meet
    
    # Solve for one variable in terms of the other
    if abs(b) > 1e-10:  # Solve for y in terms of x: y = -(ax + c)/b
        # Substitute y = -(ax + c)/b into first circle equation
        ...
        if abs(discriminant) < 1e-10:  # Single intersection point
            return [(x1, y1)]
        else:  # Two intersection points
            return [(x1, y1), (x2, y2)]
    
    else:  # Solve for x in terms of y: x = -(by + c)/a
...
def get_intersection_between_two_circles(A1, B1, C1, A2, B2, C2):
    """Get intersection points between two circles using expanded form coefficients."""
    return circle_intersections_expanded(A1, B1, C1, A2, B2, C2)
